...
import { FileText, FilePen, Search, ChevronDown, Eye } from 'lucide-react'
      ...
      <div className="p-4 lg:p-8 max-w-6xl mx-auto pb-12 pt-16 lg:pt-8">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-zinc-500 text-sm mt-1">{docs.length} total documents</p>
          </div>
        ...
        <div className="glass-card rounded-2xl overflow-hidden">
          ...
          {loading ? (
            <div className="p-6 space-y-3">
              {[1, 2, 3, 4].map((i) => <div key={i} className="h-12 bg-white/3 rounded-xl animate-pulse" />)}
            </div>
          ) : filtered.length === 0 ? (
            <div className="py-20 text-center">
              <p className="text-zinc-600 text-sm">No documents found.</p>
              <Link href="/documents/new?type=invoice" className="text-purple-400 text-sm mt-2 inline-block hover:underline">
                Create your first →
              </Link>
            </div>
          ) : (
            <div className="divide-y divide-white/3">
              {filtered.map((doc) => (
                <div
                  key={doc.id}
                  className="grid grid-cols-12 items-center px-5 py-3.5 hover:bg-white/2 transition-colors group"
                >
                  ...
                  <div className="col-span-2">
                    <span className="text-xs text-zinc-500 font-mono">
                      {doc.invoice_number || doc.id.slice(0, 8).toUpperCase()}
                    </span>
                  </div>
                  <div className="col-span-2">
                  ...
                  <div className="col-span-2 text-right">
                    <span className="text-sm font-medium text-zinc-300">${(doc.total_amount || 0).toFixed(2)}</span>
                  </div>
                  <div className="col-span-2 text-right">
