...
export default function Clients() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    await supabase.from('clients').insert([{ ...form, profile_id: profile?.id }])
    ...
    <AppLayout>
      <div className="p-6 lg:p-8 max-w-5xl mx-auto pb-12">
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            <h1 className="text-3xl font-bold text-gradient-silver tracking-tight">Clients</h1>
            <p className="text-zinc-500 text-sm mt-1">Manage saved client profiles for quick autofill</p>
          </div>
          <button
            onClick={() => setShowForm(!showForm)}
            className="glow-button-primary flex items-center space-x-2 px-4 py-2.5 rounded-xl text-sm font-medium"
          >
        ...
        {/* Add Client Form */}
        {showForm && (
          <form onSubmit={handleCreate} className="glass-card rounded-2xl p-6 mb-6 space-y-4">
            ...
            <div className="flex gap-3 pt-2">
              ...
              <button type="button" onClick={() => setShowForm(false)} className="glow-button px-5 py-2 rounded-xl text-sm font-medium">
                Cancel
              </button>
            </div>
          </form>
        ...
        {loading ? (
          ...
          <div className="glass-card rounded-2xl py-16 text-center">
            ...
            <button onClick={() => setShowForm(true)} className="text-purple-400 text-sm mt-2 hover:underline">
          ...
          <div className="space-y-2">
            {filtered.map((client) => (
              <div
                key={client.id}
                className="glass-card rounded-2xl p-4 flex items-center justify-between group hover:border-white/15 transition-all"
              >
                <div className="flex items-center space-x-4">
                  <div className="w-10 h-10 rounded-full bg-gradient-to-br from-purple-500/40 to-purple-800/40 border border-purple-500/20 flex items-center justify-center flex-shrink-0">
                ...
                <button
                  ...
                  className="p-2 text-zinc-600 hover:text-red-400 hover:bg-red-500/10 rounded-lg transition-all opacity-0 group-hover:opacity-100 border border-transparent hover:border-red-500/20"
                >
